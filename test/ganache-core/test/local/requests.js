...
const tests = function(web3) {
    ...
    describe("eth_sendTransaction", function() {
        ...
        it("should fail with bad nonce (too high)", async function() {
            ...
            try {
                ...
                assert(
                    e.message.indexOf(
                        "the tx doesn't have the correct nonce. account has nonce of: 1 tx has nonce of: 255"
                    ) >= 0
                );
            }
        });
    ...
    describe("eth_sendRawTransaction", function() {
        ...
        it("should fail with bad nonce (too high)", async function() {
            ...
            try {
                ...
                assert(
                    err.message.indexOf(
                        "the tx doesn't have the correct nonce. account has nonce of: 2 tx has nonce of: 255"
                    ) >= 0
                );
            }
        });
        ...
        it("should succeed with wrong v value (temp until next breaking change)", async function() {
            ...
            assert(
                require("../../package.json").version.startsWith("2."),
                "Version 3 should change this so the test fails!"
            );
        });
    });
    ...
    describe("contract scenario", function() {
        ...
        it("should send a state changing transaction (eth_sendTransaction)", async function() {
            ...
            assert.strictEqual(
                typeof receipt.logs[0].removed,
                "boolean",
                "Logs removed field missing or not a boolean"
            );
        ...
        it("should get only receipts relevant to the transaction (eth_getTransactionReceipt)", async function() {
            ...
            const pendingTransactionHashes = [
                send("eth_sendTransaction", [txData]),
                send("eth_sendTransaction", [txData])
            ];
            ...
            receipts.forEach((receipt) => {
                ...
                assert.strictEqual(
                    receiptBlockNumber,
                    blockNumber,
                    "Receipt blockNumber doesn't match expected block number"
                );
